...
from app.ai.models import AgentName, AnalysisRequest, AnalysisResult, ExecutiveAnalysisOutput
from app.ai.prompt_loader import PromptLoader

_PROMPT_NAME = "executive_v1"


def _build_user_message(request: AnalysisRequest, prior: AnalysisResult) -> str:
    """Serializes the mission, its datasets, and the prior Business/Strategy/Risk
...
class ExecutiveAgent:
    ...
    Business, Strategy, and Risk agents' completed analyses into a concise
    executive report.
    ...
    async def analyze(
        ...
        if (
            prior is None
            or prior.business_analysis is None
            or prior.strategy_analysis is None
            or prior.risk_analysis is None
        ):
            raise AIException(
                "ExecutiveAgent requires completed BusinessAnalysisOutput, "
                "StrategyAnalysisOutput, and RiskAnalysisOutput from prior stages."
            )
        ...
        if not raw_response or not raw_response.strip():
            raise ParsingException("Executive Agent received an empty response from the model.")

        output = parser.parse_structured_response(raw_response, ExecutiveAnalysisOutput)
