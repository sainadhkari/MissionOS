  ...
  return (
    <Card>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-sm font-semibold text-neutral-900">Risk</h2>
            ...
            <ul className="flex flex-col gap-3">
              {analysis.critical_risks.map((risk, index) => (
                <li key={index} className="rounded-md border border-neutral-200 p-3">
                  ...
                  <p className="mt-2 text-sm text-neutral-700">{risk.impact}</p>
                  <p className="mt-1.5 text-sm text-neutral-500">
                    <span className="font-medium text-neutral-600">Mitigation: </span>
                    {risk.mitigation}
                  </p>
                </li>
              ))}
